...
if symbol == "BTC/USDT":
    try:
        ...
        st.metric("Harga BTC/IDR (via Binance)", fmt_idr(btc_idr_price))

        ohlcv_usdt = binance.fetch_ohlcv("BTC/USDT", "1d", to_ms(start_dt), to_ms(end_dt))
        df_usdt = ohlcv_to_df(ohlcv_usdt)
        df_usdt[["open", "high", "low", "close"]] *= usdt_idr
        df = df_usdt
    except Exception as e:
