...
# Exchange client Tokocrypto via ccxt
class ExchangeClient:
    ...
    def resolve_symbol(self, preferred: List[str]) -> str:
        try:
            ...
            logger.warning(f"Gagal load_markets Tokocrypto: {e}. Menggunakan fallback Binance BTC/USDT")
...
# Binance multi-endpoint fallback
def get_binance_client():
    """
    Membuat client Binance dengan fallback multi-endpoint.
    Urutan percobaan: api.binance.com → api1 → api2 → api3 → api4
    """
    endpoints = [
    ...
    for ep in endpoints:
        try:
            b = ccxt.binance()
            # Set endpoint publik & privat (privat tidak dipakai untuk public data)
            b.urls["api"]["public"] = ep
            b.urls["api"]["private"] = ep
            # Tes koneksi ringan
            _ = b.fetch_ticker("BTC/USDT")
            logger.info(f"Binance endpoint aktif: {ep}")
            return b
        except Exception as e:
            logger.warning(f"Gagal konek ke {ep}: {e}")
            continue
    raise RuntimeError("Semua endpoint Binance gagal diakses.")
...
def run_dca_backtest(df: pd.DataFrame, cfg: StrategyConfig, window_years: int) -> BacktestReport:
    ...
    total_return_pct = (current_value - total_invested) / total_invested * 100 if total_invested > 0 else 0.0
    years = window_years if window_years > 0 else 1
    annualized_return_pct = ((current_value / total_invested) ** (1 / years) - 1) * 100 if total_invested > 0 else 0.0

    # Equity curve
    running_btc = 0.0
    buy_dates = set([m["buy_date"] for m in monthly_breakdown])
    for _, row in df_window.iterrows():
        date = row["time"]
        price = row[cfg.price_source]
        if date in buy_dates:
            running_btc += cfg.capital_per_trade / (price * (1 + cfg.slippage_bps / 10000.0))
        equity_curve.append({"time": date, "equity_idr": running_btc * price})
    ...
    return BacktestReport(
        ...
        total_btc=total_btc,
        avg_price_idr=total_invested / total_btc if total_btc > 0 else 0.0,
...
# Ambil kredensial dari environment (opsional untuk Tokocrypto)
...
# Resolve simbol (BTC/IDR di Tokocrypto atau fallback BTC/USDT)
symbol = client.resolve_symbol(["BTC/IDR", "BTC/IDRT", "BTC/BIDR"])
ticker = client.fetch_ticker_safe([symbol])

# Ambil harga & data OHLCV dengan fallback
end_dt = now_jakarta()
...
if symbol == "BTC/USDT":
    try:
        ...
        # Ambil harga BTC/USDT dan USDT/IDR
        btc_usdt = binance.fetch_ticker("BTC/USDT")["last"]
        usdt_idr = binance.fetch_ticker("USDT/IDR")["last"]
        btc_idr_price = btc_usdt * usdt_idr
        st.metric("Harga BTC/IDR (via Binance fallback)", fmt_idr(btc_idr_price))

        # Ambil OHLCV BTC/USDT (batasi dengan since agar efisien)
        ohlcv_usdt = binance.fetch_ohlcv(
            "BTC/USDT",
            timeframe="1d",
            since=to_ms(start_dt),
            limit=1000
        )
        df_usdt = ohlcv_to_df(ohlcv_usdt)
        # Konversi ke IDR menggunakan kurs USDT/IDR terbaru
        df_usdt[["open", "high", "low", "close"]] = df_usdt[["open", "high", "low", "close"]] * usdt_idr
        df = df_usdt
    except Exception as e:
        st.error(f"Gagal mengambil data fallback Binance: {e}")
        st.stop()
else:
    ...
    try:
        ...
        st.error(f"Gagal mengambil OHLCV dari Tokocrypto: {e}")
        st.stop()

# Validasi data
if df.empty or df["time"].max() is None:
    st.error("Data harga kosong atau tidak valid. Coba refresh atau ganti koneksi.")
    st.stop()
...
bh_results = []
for win in window_list:
    start_bh = df["time"].max() - relativedelta(years=win)
    start_price_row = first_price_on_or_after(df, start_bh, "open")
